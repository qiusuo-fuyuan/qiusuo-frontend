 ...
 * @returns {Object}
 */
export const omit = (obj: { [key: string]: Object }, arr: Array<string>) =>
  Object.keys(obj).reduce((res: { [key: string]: Object }, key) => {
...
export const parseAsURL = (text: string) => {
  if (!urlParser) {
    urlParser = window.document.createElement('a');
  }
...
export const rslError = (errorObject: RSLError) => {
  ...
  error.push(
    `[${errorObject.provider}][${errorObject.type}] ${errorObject.description}`
  );
...
export const timestampFromNow = (duration: number) => {
